...
def test_webcam(model, camera_type, camera_address=None):
    # video_capture = cv2.VideoCapture(id)
    if camera_type == 'webcam':
        video_capture = cv2.VideoCapture(0)  # Use webcam (change the index if multiple webcams are connected)
        ...
        raise ValueError("Invalid camera type. Choose 'webcam' or 'basler'.")
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Run inference on a camera feed.')
    parser.add_argument('camera_type', choices=['webcam', 'basler', 'phone'],
                        help='Type of camera to use (webcam, basler, or phone)')
    parser.add_argument('camera_address', nargs='?',
                        help='Camera address (required for phone camera, or index of a webcam)')
    args = parser.parse_args()
    
    if args.camera_type == 'webcam' and args.camera_address is not None:
        try:
            args.camera_address = int(args.camera_address)
        except ValueError:
            raise ValueError("Webcam index must be an integer.")
        
    return args

args = parse_arguments()
test_webcam(model, args.camera_type, args.camera_address)
